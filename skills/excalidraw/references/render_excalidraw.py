...
Usage:
    ...
    uv run --project "$SKILL_DIR" python "$SKILL_DIR/render_excalidraw.py" <path-to-file.excalidraw> [--output path.png] [--scale 2] [--width 1920]
...
def render(
    ...
    try:
        ...
        print('Run: SKILL_DIR="$(git rev-parse --show-toplevel)/.claude/skills/excalidraw/references" && uv sync --project "$SKILL_DIR" && uv run --project "$SKILL_DIR" playwright install chromium', file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print('Run: SKILL_DIR="$(git rev-parse --show-toplevel)/.claude/skills/excalidraw/references" && uv run --project "$SKILL_DIR" playwright install chromium', file=sys.stderr)
                sys.exit(1)
            raise
