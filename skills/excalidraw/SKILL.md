...
### Step 4: Sketch the Flow and Declare Alignment
Before JSON, mentally trace how the eye moves through the diagram. There should be a clear visual story.

Then **declare your alignment lines**: which elements form a row? What is the shared `cy`? Which elements form a column? What is the shared `cx`? Write these down before generating any coordinates — they become the source of truth for every `y` and `x` value you produce.
...
Elements in the same visual row or column must share a common center line — this is how the human eye judges "aligned." Excalidraw has no auto-align, so you must calculate positions explicitly.

- **Horizontal row** (left→right flow): pick a shared `cy`. For each element, set `y = cy - height/2`.
- **Vertical column** (top→bottom): pick a shared `cx`. For each element, set `x = cx - width/2`.
- **Grouped elements**: when shifting a container, apply the same y-delta (or x-delta) to every visual child (titles, subtitles, inner boxes, labels). Children are not auto-positioned — each has its own absolute `x`/`y`.
- **Connecting arrows**: horizontal arrows between aligned elements should have `height: 0` and share the same `y` as the row's `cy`. Vertical arrows should have `width: 0` and share the same `x`.
- **Title text**: a diagram title should be horizontally centered over the full content span. Calculate `title_x = (leftmost_x + rightmost_right) / 2 - title_width / 2`.

**Before writing any JSON**, declare your alignment lines as comments in your plan:
```
Row alignment: cy = 240
Title center: cx = 546
```
Then derive every element's y (or x) from these shared values.
...
Settings: `fontSize: 16`, `fontFamily: 6`, `textAlign: "center"`, `verticalAlign: "middle"`

**Korean text**: Use `fontFamily: 6` (Nunito) which includes Xiaolai CJK fallback for native Korean (Hangul) rendering. Nunito gives clean sans-serif readability ideal for educational/professional diagrams. Never use `fontFamily: 3` (Cascadia) — it has no Korean glyphs and will break in exports.
...
```bash
SKILL_DIR="$(git rev-parse --show-toplevel)/.claude/skills/excalidraw/references"
uv run --project "$SKILL_DIR" python "$SKILL_DIR/render_excalidraw.py" <path-to-file.excalidraw>
```
...
- Misaligned centers: elements in the same row have visibly different vertical positions, or elements in the same column have different horizontal positions
...
uv run --project "$SKILL_DIR" playwright install chromium
```
...
16. **Row/column alignment**: Elements in the same row share identical cy; elements in the same column share identical cx
